...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h3 class="m-0 font-weight-bold text-primary">Tambah Tanda Penghormatan</h3>
    </div>
    <div class="card-body">
        <form action="{{ route('personil.tankeh.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('post')
            <!-- Tanda Kehormatan -->
            ...
            <div class="form-group">
                ...
                <input type="tmt" name="tmt" class="form-control @error('tmt') is-invalid @enderror" value="{{ old('tmt') }}">
                @error('tmt')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
